...
class APIKeyDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("X-ray智能系统登录")
        self.setMinimumSize(500, 300)  # 增加对话框大小
        self.api_key = ""
        self.initUI()
    ...
    def initUI(self):
        ...
        # 标题
        title_label = QLabel("X-ray智能系统")
        title_label.setAlignment(Qt.AlignCenter)
        title_label.setObjectName("dialogTitle")
        
        # 说明文字
        info_label = QLabel("请输入您的密钥以使用骨科X-ray智能分析系统。")
        info_label.setWordWrap(True)
        info_label.setAlignment(Qt.AlignCenter)
        info_label.setObjectName("infoText")
        
        # API密钥输入框
        input_group = QGroupBox("系统密钥")
        input_layout = QVBoxLayout()
        
        self.api_key_edit = QLineEdit()
        self.api_key_edit.setPlaceholderText("请输入您的密钥")
        self.api_key_edit.setEchoMode(QLineEdit.Password)
        self.api_key_edit.setObjectName("apiKeyInput")
        
        input_layout.addWidget(self.api_key_edit)
        ...
        self.ok_btn.setObjectName("actionButton")
        ...
        # 帮助文本
        help_label = QLabel("如需获取密钥，请向管理员申请")
    ...
    def accept(self):
        ...
        if not api_key:
            QMessageBox.warning(self, "警告", "请输入有效的系统密钥")
            return
            
        if not api_key.startswith('sk-'):
            QMessageBox.warning(self, "警告", "系统密钥格式不正确，应以'sk-'开头")
            return
...
class MedicalVisionApp(QMainWindow):
    def __init__(self):
        ...
        # 初始时禁用主界面功能
        self.enable_main_interface(False)
    ...
    def show_api_key_dialog(self):
        ...
        dialog.setWindowIcon(self.windowIcon())  # 使用主窗口图标
        result = dialog.exec_()
        
        if result == QDialog.Accepted:
            api_key = dialog.get_api_key()
            self.deepseek_expert = DeepSeekOrthoExpert(api_key)
            self.status_bar.showMessage("密钥输入成功", 3000)
            # 启用主界面功能
            self.enable_main_interface(True)
        else:
            # 用户取消或关闭对话框
            reply = QMessageBox.question(
                self, 
                "确认退出", 
                "需要有效的密钥才能使用系统功能。是否退出应用？",
    ...
    # 添加启用/禁用主界面功能的方法
    def enable_main_interface(self, enabled):
        """启用或禁用主界面控件"""
        self.upload_btn.setEnabled(enabled)
        self.upload_dicom_btn.setEnabled(enabled)
        self.analyze_btn.setEnabled(enabled)
        self.deepseek_btn.setEnabled(enabled)
        ...
        if not enabled:
            ...
            self.deepseek_display.setText("请先输入密钥以使用系统功能")
